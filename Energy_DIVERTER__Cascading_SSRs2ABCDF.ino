...
void SWITCH(int Inp){
  ...
  if (Inp > 310 && Inp < 340 ){Serial.println("switch1 off");digitalWrite(relay1, LOW); EEPROM.update(address1, 10 ); }
  if (Inp > 280 && Inp < 310 ){Serial.println("switch1 on");digitalWrite(relay1, HIGH); EEPROM.update(address1, 10 ); }
  
  if (Inp > 250 && Inp < 280 ){ Serial.println("switch2 auto");sw2=3; EEPROM.update(address2, sw2 );}
  if (Inp > 220 && Inp < 250){Serial.println("switch2 off");digitalWrite(relay2, LOW);EEPROM.update(address2, 10 ); }
  if (Inp > 190 && Inp < 220 ){Serial.println("switch2 on");digitalWrite(relay2, HIGH);  EEPROM.update(address2, 10 ); }
  
  if (Inp > 160 && Inp < 190 ){Serial.println("switch3 auto");sw3=3; EEPROM.update(address3, sw3 );}
  if (Inp > 130 && Inp < 160 ){Serial.println("switch3 off");digitalWrite(relay3, LOW); EEPROM.update(address3, 10 );}
  if (Inp > 100 && Inp < 130 ){Serial.println("switch3 on");digitalWrite(relay3, HIGH); EEPROM.update(address3, 10 ); }
  
  if (Inp > 70 && Inp < 100 ){Serial.println("switch4 auto"); sw4=3; EEPROM.update(address4, sw4 );}
  if (Inp > 40 && Inp < 70 ){Serial.println("switch4 off");digitalWrite(relay4, LOW); EEPROM.update(address4, 10 );}
  if (Inp > 10 && Inp < 40 ){Serial.println("switch4 on");digitalWrite(relay4, HIGH);EEPROM.update(address4, 10 );} 
